...
class SetupCreator
  ...
  def read_sw_version()
    script_fname = File.expand_path("../src/App/MenuMgr.cpp")
    File.open(script_fname, "r").each_line do |line|
      #p line
      # search a line with g_lpszVersion. It is something like:
      # g_lpszVersion = \"Ver 2.0.1 20230403\"
      if line =~ /g_lpszVersion(.*)/
        arr_tmp = $1.split("\"")
        arr_tmp.each do |tmp_str|
          if tmp_str =~ /Ver(.*)/
            # expect here something like " 2.0.1 20230403"
        ...
        log("Error on parsing g_lpszVersion")
        return
      end
    end
    log("Error g_lpszVersion not found")
  end
  ...
  def create_nsi_installer_script(target_dir, app_data_fullpath)
    ...
    nsi_out_name = File.join(target_dir, "solitario_gen.nsi")
