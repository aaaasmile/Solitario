...
void sdlMenu::AddItems(string item) { Items.push_back(item); }

void sdlMenu::ClearItems() { Items.clear(); }
  ...
  while (bEnd == false) {
    if (bRedraw == true) {
      ...
      for (unsigned int k = 0; k < Items.size(); k++) {
        ...
        Font->DrawString(Screen, Items[k], TEXTMIXED, TEXTALIGNCENTER, 0,
                         Rect.y + (25 + SDLFONTSIZE) + ((SDLFONTSIZE + 5) * k),
                         0);
      }
        ...
        case SDL_KEYDOWN:
          switch (event.key.keysym.sym) {
            case SDLK_ESCAPE:
              ...
              break;
            case SDLK_RETURN:
              if (iCurPos == Items.size() - 1) iCurPos = -1;
              bEnd = true;
              break;
            case SDLK_UP:
              if (iCurPos == 0)
...
void sdlMenu::SetBitmap(string filename) { FilenameBackground = filename; }
...
SDL_Rect *sdlMenu::GetArea() { return &Rect; }
...
*****************************************************************************************************************************
*/
void sdlMenu::DrawBitmap(unsigned char alignment, Uint32 color, SDL_Surface *screen) {
  SDL_Rect s, t;

  //SDL_Surface *screen = SDL_GetVideoSurface();

  if ((Bitmap == NULL) || (screen == NULL)) return;
...
//     SDL_SetColors(screen, Bitmap->format->palette->colors, 0, // TODO SDL 2.0
